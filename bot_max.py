...
def welcome_text() -> str:
    """Первое сообщение бота: что это и как получить цену (кнопка «ХОЧУ»).

    Текст задан Ириной 24.09. Реквизиты — следующим шагом по кнопке,
    поэтому в приветствии только цена-ориентир и один призыв к действию.
    """
    return (
        "Это бот пошагового гайда «Установка ИИ-агента».\n\n"
        ...
        "«Напиши письмо клиенту». И он делает. Без ChatGPT. Без блокировок. Без нервов.\n\n"
        "Это CodeWhale — твой личный ИИ-помощник\n"
        "💸 Сколько стоит? В 10 раз дешевле Claude Code и Codex. "
        "Никаких подписок по $20-200. Никаких списаний за месяц простоя.\n"
        ...
        "Пошаговая инструкция по установке CodeWhale и голосового управления "
        "к нему → жми «💳 ХОЧУ методичку»\n"
...
def _buy_or_help_text() -> str:
    ...
    Текст задан Ириной 24.09 — совпадает с приветствием: что это за бот
    и один шаг — кнопка. Слова-угадайки («хочу», «сколько стоит», «да»)
    убраны: шаг покупки идёт через кнопку, а не через разбор текста.
    """
    return (
